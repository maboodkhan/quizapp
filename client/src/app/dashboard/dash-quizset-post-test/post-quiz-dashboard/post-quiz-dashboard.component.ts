...
import { PostQuizInfoDashComponent } from '../post-quiz-info-dash/post-quiz-info-dash.component';
import { PostDashboardService } from '../../post-dashboard.service';

@Component({
  selector: 'app-post-quiz-dashboard',
  templateUrl: './post-quiz-dashboard.component.html',
  styleUrls: ['./post-quiz-dashboard.component.css']
})
export class PostQuizDashboardComponent implements OnInit {
  ...
  getQuizSets(){
    this.postDashboardService.getDasboardQuizSets(this.customFilters).subscribe((quizSets: any) => {
...
openDialog(quiz_set_id) {
  ...
  this.dialog.open(PostQuizInfoDashComponent, dialogConfig);
}
}
