...
module.exports = function (Quizset) {
    ...
    let quizSetSchoolsCreate = (school_id, quiz_set_id) => {
        return new Promise((resolve, reject) => {
            let quizSetSchool = Quizset.app.models.quiz_set_schools;
            quizSetSchool.update({quiz_set_id: quiz_set_id},{status:2},
                function(err, res){
                    if (err) {
                        reject(err);
                    }
                    school_id.forEach(schoolId => {
                        ...
                        quizSetSchool.upsertWithWhere({ quiz_set_id: quiz_set_id, school_id: schoolId }, obj,
                            function (err, result) {
                                if (err) {
                                    reject(err);
                                }
    ...
    let quizSyllabusCreate = (quiz_set_id, topic_ids, section_ids, country_id) => {
        return new Promise((resolve, reject) => {
            let quizSyllabus = Quizset.app.models.quiz_syllabus_details;
            let topicLessonId;
            let lessonSubjectId;
            let subjectClassId;
            let classBoardId;
    ...
    let quizSyllabusCountryCreate = (quiz_set_id, topic_ids, section_ids, country_id, country_lesson_id) => {
        return new Promise((resolve, reject) => {
            let quizSyllabus = Quizset.app.models.quiz_syllabus_details;
            let topicLessonId;
            let lessonSubjectId;
            ...
            quizSyllabus.destroyAll({ quiz_set_id: quiz_set_id }, function (err, delResult) {
                ...
                let Subject = Quizset.app.models.subjects;
                let Class = Quizset.app.models.classes;
                let LessonMapping = Quizset.app.models.country_lesson_mapping;
                let countryLessons = Quizset.app.models.country_lessons;
                section_ids.forEach(async (section_id) => {
    ...
    let quizQuesCreate = (quiz_set_id, questions) => {
        return new Promise((resolve, reject) => {
            let quizQuestion = Quizset.app.models.quiz_questions;
            let topicLessonId;
            let lessonSubjectId;
            let subjectClassId;
            let classBoardId;
            quizQuestion.destroyAll({ quiz_set_id: quiz_set_id }, function (err, delResult) {
                ...
                questions.forEach((quesVal) => {
                    let quesData = { quiz_set_id: quiz_set_id, question_id: quesVal };
    ...
    let getQuizSetLists = (set_name, set_id, school_id, class_id, section_id, type_order, created_by, created_by_id, limit, offset) => {
        return new Promise((resolve, reject) => {
            ...
            //     }
                ...
                // FROM quiz_set qs 
                // LEFT JOIN quiz_set_schools qss on qs.id = qss.quiz_set_id
                // LEFT JOIN quiz_syllabus_details qsyb on qs.id = qsyb.quiz_set_id
                ...
                var sql = `SELECT qs.id as quiz_set_id, qs.set_name, qs.created_on, qs.created_by, qs.status,
                qsyb.class_id, c.class_name
                FROM quiz_set qs 
                LEFT JOIN quiz_set_schools qss on qs.id = qss.quiz_set_id
                LEFT JOIN quiz_syllabus_details qsyb on qs.id = qsyb.quiz_set_id
    ...
    Quizset.getDasboardQuizSets = function (data, cb) {
        ...
        if (student_id !== undefined) {
            joinCond = joinCond + " JOIN attempted_quiz_set aqs on qs.id = aqs.quiz_set_id";
            ...
            joinCond = joinCond + " JOIN quiz_set_schools qss on qs.id = qss.quiz_set_id";
        ...
        var sql = `SELECT qs.id as quiz_set_id, qs.set_name, qs.status, qs.created_by
    ...
    Quizset.getTotalQuiz = function (data, cb) {
        ...
        JOIN quiz_set_schools qss on qs.id = qss.quiz_set_id
        JOIN quiz_syllabus_details qsyb on qs.id = qsyb.quiz_set_id
        ...
        if (section_id === undefined) {
            ...
            join quiz_set qs on qs.id = aqs.quiz_set_id
            join quiz_user qu on qu.id = aqs.user_id
            join user_data ud on qu.email = ud.email
            join quiz_set_schools qss on qs.id = qss.quiz_set_id
            join quiz_syllabus_details qsyb on qsyb.quiz_set_id = qs.id
    ...
    Quizset.getQuizTopic = function (data, cb) {
        ...
        if(user_id != undefined){
            joinCond = `JOIN attempted_quiz_set aqs on aqs.quiz_set_id = qsyb.quiz_set_id`
            cond = ` and aqs.user_id = ${user_id}`;
        }

        var countSql = `SELECT qsyb.id FROM quiz_syllabus_details qsyb
        JOIN quiz_set_schools qss on qsyb.quiz_set_id = qss.quiz_set_id
    ...
    let getCountryLessonDetails = (country_lesson_id) => {
        return new Promise((resolve, reject) => {
            let msg = {};
            let Lesson = Quizset.app.models.country_lessons;
            Lesson.findOne(
                {
                    where: {
    ...
    let getMappedCountryLessonDetails = (lesson_id) => {
        return new Promise((resolve, reject) => {
            let msg = {};
            let Lesson = Quizset.app.models.country_lesson_mapping;
            Lesson.findOne(
                {
                    where: {
                        lesson_id: lesson_id
