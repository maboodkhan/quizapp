...
module.exports = function(Quizsetschools) {
    ...
    Quizsetschools.getQuizSets = function(data, cb){
        ...
        var countSql = `SELECT count(*) as totalCount FROM quiz_set qs 
            ...
            and qs.id NOT IN(select aqs.quiz_set_id from attempted_quiz_set aqs, user_data ud 
                where aqs.user_id = ud.user_id and ud.email = '${email}'  and qs.multiple_attempts = 0)    
            ${cond}
            GROUP by qs.id, qss.school_id;`;

        var sql = `SELECT qs.id as quiz_set_id, qs.set_name, qs.quiz_syllabus_path, qs.num_ques, 
            qs.duration, qs.multiple_attempts, qss.publishDate, sch.school_name, sch.school_code, qsyb.class_id, c.class_name, 
    ...
    Quizsetschools.getUserQuizSets = function(data, cb){
        let msg = {};        
        let school_id = (data.school_id==undefined || data.school_id==null || data.school_id==''?0:data.school_id);
        let email = (data.email==undefined || data.email==null || data.email==''?'':data.email);
        let set_name = (data.setName==undefined || data.setName==null || data.setName==''?undefined:data.setName);
        ...
        if(set_name !== undefined) {
            cond = cond + " and ( qs.set_name like '%"+ set_name +"%')";
        }
    ...
    Quizsetschools.remoteMethod(
        'getUserQuizSets',
        {
            http: {path: '/getuserquizsets', verb: 'post'},
            description: 'Get user non attempted quiz sets',
            accepts: [{arg: 'data', type: 'object', http: {source: 'body'}}],
            returns: {root: true, type: 'json'}
        }
    );
